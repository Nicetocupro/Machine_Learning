...
import models.LassoRegression as lassor
...
class TyphoonDataProcessor:
    def __init__(self, file_path):
        ...
        # self.model = lassor.LassoRegression()
    ...
    def load_data(self):
        ...
        typhoons = [self.data[self.data["International number ID"] == ID].drop(["International number ID"], axis=1) for
                    ID in ids]
    ...
    def split_data(self):
        ...
        print("验证集特征形状:", self.x_val.shape)
        print("验证集标签形状:", self.y_val.shape)
    ...
    def train_model(self):
        ...
        print("Training LinearRegression model method=matrix")
        self.model.train(self.x_train, self.y_train, method="matrix", learning_rate=0.1, n_iters=5000)

        """
        print("Training LinearRegression model method=gradient")
        self.model.train(self.x_train, self.y_train, method="gradient", learning_rate=0.1, n_iters=5000)
        """
        """
        # 下面是岭回归的测试
        print("Training RidgeRegression model")
        self.model.train(self.x_train, self.y_train, lambdas=0.1)
        """
        """
        # 下面是lasso回归的测试
        print("Training LassoRegression model method=coordinate_descent")
        self.model.train(self.x_train, self.y_train, method='coordinate_descent', lambdas=0.01, n_iters=5000)
        """
        """
        print("Training LassoRegression model method=least_angle_regression")
        self.model.train(self.x_train, self.y_train, method='least_angle_regression', lambdas=0.01, n_iters=5000)
        """
    ...
    def visualize_sample(self):
        ...
        folium.Circle(location=[x_sample[0][5], x_sample[0][6]], radius=x_sample[0][11] * 1852, fill=True,
                      color="black", fill_color="yellow").add_to(m)
        folium.Circle(location=[x_sample[1][5], x_sample[1][6]], radius=x_sample[1][11] * 1852, fill=True,
                      color="black", fill_color="yellow").add_to(m)
        folium.Circle(location=[y_sample[0], y_sample[1]], radius=y_sample[-2] * 1852, fill=True, color="purple",
                      fill_color="orange").add_to(m)
        folium.Circle(location=[lr_pred_sample[0], lr_pred_sample[1]], radius=lr_pred_sample[-2] * 1852, fill=True,
                      color="blue", fill_color="red").add_to(m)
        return m


# Usage
def main():
    processor = TyphoonDataProcessor("../data/typhoon_data.csv")
    processor.load_data()
